...
// this is the start of the doc specific context info
    ...
    intent.on('time', function() {
      //Set an event that will print the current time context
      $('#timeExample').text(intent.axes.time.current);
    });

    $('#timeChange').change(function() {
      //Let's users emulate time of day
      //Default to actual time when no specification
      if($(this).children('option:selected').val() == '') { time.respond(); }
      //Otherwise pass Intention the selected string
      else { time.respond($(this).children('option:selected').val()); }
      //If the users on a small device, scroll to the change
      if(intent.axes.width.current == 'mobile' ||
         intent.axes.width.current=='smalltablet') {
	...
	var depth = $('#'+intent.axes.titleDepth.contexts[i]['name'])
          .offset().top - 50;
    ...
    $('.jump').add('#topNav .inner p a').click(function() {
